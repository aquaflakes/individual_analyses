...
p=ggplot(data = plotdf,aes(factor(label,levels = c("mode2_ratio_T","mode1_ratio_T","mode2_ratio_Ctrls","mode1_ratio_Ctrls")),log2_b_ub))+ geom_point(size=0.8,alpha=0.99)
p=p+ stat_summary(color="red",alpha=0.5) + gg_axis_x_labels(c("Mode 2 (T)", "Bk (T)", "Mode 2 (Rnd)", "Bk (Rnd)")) +
